...
export function convertAmountToNumber(value, decimals = 18) {
  return new BigNumber(value)
...
export function convertAmountFromRawNumber(value, decimals = 18) {
  if (decimals == 8) {
    return new BigNumber(`${parseInt(value)}`)
...
export function convertAmountFromNumber(value, decimals = 18) {
  if (decimals == 6) {
    return new BigNumber(parseInt(value))
      .multipliedBy(new BigNumber(10).exponentiatedBy(decimals))
      .toString(10);
...
export function calculateReallyNum(total, sliderNum) {
  if (sliderNum === undefined) {
    return byDecimals(0, 0).toFormat(2);
  }
  return byDecimals((sliderNum / 100) * Number(total), 0).toFormat(2);
}
